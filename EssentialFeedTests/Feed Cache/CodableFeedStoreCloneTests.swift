...
class CodableFeedStoreCloneTests: XCTestCase {
    ...
    func test_retrieve_deliversEmptyOnEmptyCache(){
        ...
        sut.retrieve { result in
            ...
            default:
                XCTFail("Expected empty result, got a \(result) instead")
            }
            exp.fulfill()
        ...
        sut.insert(feed, timestamp: timestamp) { insertionError in
            ...
            sut.retrieve { firstResult in
                sut.retrieve { secondResult in
                    ...
                    default:
                        XCTFail("Expected retrieving twice from non empty cache to deliver samefound result with feed \(feed) and timestamp \(timestamp), got \(firstResult) and \(secondResult) instead")
                    }
                    exp.fulfill()
                }
    ...
    private func makeSUT(file: StaticString = #file, line: UInt = #line) -> CoddableFeedStoreClone {
        let storeURL = testSpecificStoreURLClone()
        let sut = CoddableFeedStoreClone(storeURL: storeURL)
